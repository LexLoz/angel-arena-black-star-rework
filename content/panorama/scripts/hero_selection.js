...
function HeroSelectionEnd(bImmidate) {
	$.Schedule(bImmidate ? 0 : 1.6, function () { //1.5 + 0.1
		...
		if ($('#HeroSelectionBox')) $('#HeroSelectionBox').visible = false;
		// if (HideEvent != null)
		...
		let MinimapPTIDs = $.GetContextPanel().MinimapPTIDs
		if (MinimapPTIDs && MinimapPTIDs.length > 0)
...
GameEvents.Subscribe('reconnect-fix', (function (event) {
	if (event.removePickWindow) {
		$.Msg('remove window')
...
function OnLocalPlayerPicked() {
	...
	$.CreatePanel("DOTAScenePanel", ScenePanel, "scene", { particleonly: "false", allowrotation: "true", unit: model });
...
function UpdateMainTable(tableName, changesObject, deletionsObject) {
	var selectionState = changesObject.HeroSelectionState;
	print('new state: ' + selectionState)
	if (selectionState < HERO_SELECTION_PHASE_END && changesObject.HeroTabs != null) {
	...
	print('selectionState: ' + typeof (selectionState))
	if (selectionState != null) {
		SetCurrentPhase(selectionState);
...
function SetCurrentPhase(selectionState) {
	...
	var context = $.GetContextPanel();
	context.SetHasClass('IsInBanPhase', selectionState === HERO_SELECTION_PHASE_BANNING);
	HeroSelectionState = selectionState;
	InitializationStates[selectionState] = true;
	if ($('#GameModeInfoCurrentPhase')) $('#GameModeInfoCurrentPhase').text = $.Localize('#' + 'hero_selection_phase_' + selectionState);
	UpdateSelectionButton();
}
...
(function () {
	...
	var heroSelectionEnded = PlayerTables.GetTableValue('hero_selection_available_heroes', 'HeroSelectionEnded');
	// PlayerTables.GetAllTableValues("hero_selection_available_heroes")
	// print('heroSelectionEnded: ' + heroSelectionEnded)
	// console.log('game id: ' + Game.GetLocalPlayerID())
	// console.log('player id: ' + Players.GetLocalPlayer())
	// console.log(Players.IsValidPlayerID(localPlayerId))
	if (Players.IsValidPlayerID(localPlayerId) && !Players.IsSpectator(localPlayerId) && !heroSelectionEnded) {
		_DynamicMinimapSubscribe($('#MinimapDynamicIcons'), function (ptid) {
			$.GetContextPanel().MinimapPTIDs.push(ptid);
		});
