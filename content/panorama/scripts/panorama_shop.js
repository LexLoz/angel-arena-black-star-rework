	...
	panel.SetPanelEvent('onactivate', function() {
		...
		if (GameUI.IsAltDown()) {
			GameEvents.SendCustomGameEventToServer('custom_chat_send_message', {
				shop_item_name: panel.IsInQuickbuy ? QuickBuyTarget[0] : itemName,
				isQuickbuy: panel.IsInQuickbuy,
				gold: GetRemainingPrice(panel.IsInQuickbuy ? QuickBuyTarget[0] : itemName, {})
			});
		} else {
			ShowItemRecipe(itemName);
			if (GameUI.IsShiftDown() && !GameUI.IsControlDown()) {
				SetQuickbuyTarget(itemName);
			}
			if (GameUI.IsShiftDown() && GameUI.IsControlDown()) {
				print('control')
				SetQuickBuyQueue(itemName)
			}
		}
	});
...
function RefreshQuickbuyItem(itemName) {
	MakeQuickbuyCheckItem(itemName, {}, {}, QuickBuyTargetAmount, 0);
}

function RefreshQuickbuyQueue(items) {
	for (var i in items) {
		//print('lol')
...
function MakeQuickbuyCheckItem(itemName, ItemCounter, ItemIndexer, sourceExpectedCount) {
	var RecipeData = ItemData[itemName].Recipe;
	var index = QuickBuyTarget.length - 1;
	if (ItemCounter[itemName] == null)
		ItemCounter[itemName] = 0;
	...
	if ((itemCount < ItemCounter[itemName] || (itemName === QuickBuyTarget[index || 0] && itemCount - (sourceExpectedCount - 1) < ItemCounter[itemName]))) {
		if (RecipeData != null && RecipeData.items != null) {
			_.each(RecipeData.items[1], function(childName) {
		...
		if (itemName === QuickBuyTarget[index]) {
			ClearQuickbuyItems();
...
function SetQuickBuyQueue(itemName) {
	Game.EmitSound('Quickbuy.Confirmation');
	QuickBuyTarget.push(itemName);
	QuickBuyTargetAmount = GetItemCountInCourier(Players.GetPlayerHeroEntityIndex(Game.GetLocalPlayerID()), itemName, true) + GetItemCountInInventory(Players.GetPlayerHeroEntityIndex(Game.GetLocalPlayerID()), itemName, true);
	RefreshQuickbuyQueue(QuickBuyTarget);
}
...
function AutoUpdateQuickbuy() {
	if (QuickBuyTarget.length == 1) {
		RefreshQuickbuyItem(QuickBuyTarget[0]);
	}else if (QuickBuyTarget.length > 1) {
		RefreshQuickbuyQueue(QuickBuyTarget)
	}
...
(function() {
	...
	DynamicSubscribePTListener('panorama_shop_data', function(tableName, changesObject, deletionsObject) {
		if (changesObject.ShopList) {
			...
			SetQuickbuyStickyItem('item_teleport');
		};
