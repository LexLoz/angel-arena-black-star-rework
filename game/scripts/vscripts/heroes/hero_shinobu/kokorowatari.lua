...
function ChanceToKill(keys)
	...
	local ability = keys.ability
	if not target:IsConsideredHero() and not target:IsMagicImmune() and not target:IsInvulnerable() and not target:IsBuilding() and RollPercentage(ability:GetAbilitySpecial("chance_to_kill")) then
