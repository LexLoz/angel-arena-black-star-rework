function SplitShotLaunch(keys)
	...
	local split_shot_targets = FindUnitsInRadius(caster:GetTeam(), caster:GetAbsOrigin(), nil,
		caster:Script_GetAttackRange(), ability:GetAbilityTargetTeam(), ability:GetAbilityTargetType(),
	...
	local accept_stone_gaze = false
	if medusa_stone_gaze_arena and medusa_stone_gaze_arena:GetLevel() > 0 and medusa_stone_gaze_arena:IsCooldownReady() and RollPercentage(medusa_stone_gaze_arena:GetAbilitySpecial("stone_chance_pct")) then
		medusa_stone_gaze_arena:AutoStartCooldown()
		accept_stone_gaze = true
		print('accept_stone_gaze')
	end
	...
	for _, v in ipairs(split_shot_targets) do
		if v ~= attack_target and not v:IsAttackImmune() then
			...
			if medusa_mystic_snake_arena and medusa_mystic_snake_arena:GetLevel() > 0 and medusa_mystic_snake_arena:IsCooldownReady() and not caster:PassivesDisabled() then
				...
				ParticleManager:CreateParticle("particles/units/heroes/hero_medusa/medusa_mystic_snake_cast.vpcf",
			...
			if accept_stone_gaze then
				...
				v:EmitSound("Hero_Medusa.StoneGaze.Stun")
				medusa_stone_gaze_arena:ApplyDataDrivenModifier(caster, v, "modifier_stone_gaze_stone_arena",
					{ duration = stone_duration })
			end
		end
...
function SplitShotDamage(keys)
	...
	-- print(keys.ability:GetAbilitySpecial("arrow_damage"))
	-- print(caster:GetAverageTrueAttackDamage(target))
	if not caster:IsIllusion() then
		ApplyDamage({
			attacker = caster,
			victim = target,
			damage_type = keys.ability:GetAbilityDamageType(),
			damage = caster:GetAverageTrueAttackDamage(target) * keys.ability:GetAbilitySpecial("arrow_damage") * 0.01,
