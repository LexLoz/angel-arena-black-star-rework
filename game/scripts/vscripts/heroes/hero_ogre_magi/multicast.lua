...
if IsServer() then
	function modifier_multicast_lua:OnAbilityExecuted(keys)
		...
		local strength_mult = caster:GetReliableStrength() / 20
		local multicast
		if RollPercentage(ability:GetSpecialValueFor("multicast_4_times") * multiplier + strength_mult * 0.5) then
			multicast = 4
		elseif RollPercentage(ability:GetSpecialValueFor("multicast_3_times") * multiplier + strength_mult * 0.75) then
			multicast = 3
		elseif RollPercentage(ability:GetSpecialValueFor("multicast_2_times") * multiplier + strength_mult) then
			multicast = 2
		end
