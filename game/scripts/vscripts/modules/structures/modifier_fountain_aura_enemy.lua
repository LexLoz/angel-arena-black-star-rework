...
if IsServer() then
		...
		Timers:CreateTimer(0.1, function()
			...
			if IsValidEntity(parent) then
				...
				local pfx = ParticleManager:CreateParticle(
					"particles/units/heroes/hero_lina/lina_spell_laguna_blade.vpcf", PATTACH_ABSORIGIN, fountain)
				ParticleManager:SetParticleControl(pfx, 0, fountain:GetAbsOrigin() + Vector(0, 0, 224))
				ParticleManager:SetParticleControlEnt(pfx, 1, parent, PATTACH_POINT_FOLLOW, "attach_hitloc",
	...
	function modifier_fountain_aura_enemy:OnIntervalThink()
		local parent = self:GetParent()
		if parent:IsCourier() or parent:GetName() == "infinity_stone" or parent:IsBuilding() then return end
		...
		if Bosses:IsAlive("cursed_zeld") or parent:IsCustomWard() then
			...
			ApplyInevitableDamage(
				FindFountain(self.team),
				parent,
				nil,
				1000 + parent:GetMaxHealth() * self:OnTooltip() * 0.01,
				true
			)
		end
	end
