...
if IsServer() then
	...
	function modifier_item_radiance_lua:OnIntervalThink()
		...
		if not ability.disabled then
			for _,v in ipairs(FindUnitsInRadius(target:GetTeamNumber(), target:GetAbsOrigin(), nil, ability:GetSpecialValueFor("aura_radius"), DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_NONE, FIND_ANY_ORDER, false)) do
				if not v:IsMagicImmune() and not v:IsDebuffImmune() and target:IsAlive() then
...
function modifier_item_radiance_lua_effect:GetModifierSpellLifestealRegenAmplify_Percentage()
	return self:GetAbility():GetSpecialValueFor("aura_heal_reduce")
end

function modifier_item_radiance_lua_effect:GetModifierHealAmplify_PercentageTarget()
	return self:GetAbility():GetSpecialValueFor("aura_heal_reduce")
end

function modifier_item_radiance_lua_effect:GetModifierLifestealRegenAmplify_Percentage()
	return self:GetAbility():GetSpecialValueFor("aura_heal_reduce")
end

function modifier_item_radiance_lua_effect:GetModifierHPRegenAmplify_Percentage()
	return self:GetAbility():GetSpecialValueFor("aura_heal_reduce")
end
